 ...
 * Description: hdfs HA机制测试类 仅修改url为集群的HA即可 即federation1
 **/
...
public class HdfsUtilHA {
    private final Log logger = LogFactory.getLog(HdfsUtilHA.class);
    FileSystem fs = null;

    public HdfsUtilHA() throws IOException, InterruptedException, URISyntaxException {
        Configuration conf = new Configuration();
        //fs = FileSystem.get(conf);
        //conf.set("fs.defaultFS", "hdfs://hadoop101:9000");
        fs = FileSystem.get(new URI("hdfs://federation1/"), conf, "gxm");
    }
...
//		osLocal.close();
    ...
    public boolean mkdir(String srcFile) throws IOException {
        Path srcPath = new Path(srcFile);
        if (fs.exists(srcPath)) {
            logger.warn("mkdir fail srcFile: " + srcFile + " exist");
            return false;
        }
